...
if exists (select '1' from #indexstats indexstats join sys.indexes i 
on i.object_id = indexstats.OBJECT_ID
and i.index_id = indexstats.index_id
...
declare defrag_crsr cursor fast_forward for
select distinct 'ALTER INDEX '+i.name+' ON '+object_name(indexstats.object_id)+' REBUILD WITH (SORT_IN_TEMPDB = ON, MAXDOP = 1)'
...
while 1=1
begin
  fetch defrag_crsr into @sql
  if @@fetch_status <> 0
    break

  print @sql
  print 'Start '+cast(getdate() as varchar(20))
  ...
  print 'End '+cast(getdate() as varchar(20))
end

close defrag_crsr
deallocate defrag_crsr

-- FRAGMENTATION REPORT AFTER DEFRAG

declare frag_crsr cursor fast_forward for
select distinct object_name(indexstats.object_id) 
from #indexstats

open frag_crsr

fetch frag_crsr into @table
...
fill_factor,is_padded,
is_hypothetical,allow_row_locks,allow_page_locks
from #indexstats1 indexstats left join sys.indexes i 
on i.object_id = indexstats.OBJECT_ID
and i.index_id = indexstats.index_id
and i.name is not null
order by avg_fragmentation_in_percent desc

drop table #indexstats1 
