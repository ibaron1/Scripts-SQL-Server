...
and column_name like 'ReportingDecision%'

/*********************************/

--declare @tbl varchar(200) = null
declare @tbl varchar(200) = 'EODTradeStage'

SELECT @@servername as [sql server], db_name() as [database], TABLE_SCHEMA, TABLE_NAME, column_name, 
        data_type + case data_type
            when 'sql_variant' then ''
            when 'text' then ''
            when 'ntext' then ''
...
on isc.TABLE_NAME = so.name and so.type='U' and isc.TABLE_SCHEMA = schema_name(so.schema_id)
where isc.TABLE_NAME = coalesce(@tbl, isc.TABLE_NAME)
and column_name in ('UnderlyingAssetIDType','ReferenceEntityID')
