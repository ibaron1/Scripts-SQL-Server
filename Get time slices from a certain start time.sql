declare @start datetime2 = getdate() --'2017-06-27 13:54:17.4296989'
...
avg(datediff(ms,time_before_new_order,time_after_new_order)) as [Avg sp_new_order execution in ms]
from new_order_metrics
where time_after_new_order > @start)
...
[Avg sp_new_order execution in ms]
from  run_time
select 
...
where time_after_fill_order > @start
)
select [Start of run],[End of run],
case when run_time_sec/3600 > 0 then cast(run_time_sec/3600 as varchar(2))+' hr ' else '' end+
case when (run_time_sec%3600)/60 > 0 then cast((run_time_sec%3600)/60 as varchar(2))+' min ' else '' end+
...
where time_after_get_order_summary > @start)
select [Start of run],[End of run],
case when run_time_sec/3600 > 0 then cast(run_time_sec/3600 as varchar(2))+' hr ' else '' end+
case when (run_time_sec%3600)/60 > 0 then cast((run_time_sec%3600)/60 as varchar(2))+' min ' else '' end+
cast(run_time_sec%3600-((run_time_sec%3600)/60)*60 as varchar(2))+' sec ' as [Run duration],
