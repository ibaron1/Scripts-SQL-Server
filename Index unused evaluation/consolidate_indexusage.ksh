...
create unique clustered index i1 on monitordb..indexusage(ReportDate,SrvName,DB,TableName,IndexName)
...
select 	@@servername,db_name(),
	object_name(i.id),i.name,
        case when i.status&2=2 and lockscheme(i.id, db_id())='allpages' and i.indid=1 then 'unique CI'
             when i.status&2!=2 and lockscheme(i.id, db_id())='allpages' and i.indid=1 then 'nonunique CI'
             when i.status&2=2 and lockscheme(i.id, db_id())<>'allpages' and i.indid=2 then 'unique CI'
             when i.status&2!=2 and lockscheme(i.id, db_id())<>'allpages' and i.indid=2 then 'nonunique CI'
