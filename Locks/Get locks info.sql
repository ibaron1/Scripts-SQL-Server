...
where db_name(rsc_dbid) = DB_NAME()
and rsc_objid <> 0
order by tbl
----------------------------------------------------------------

create table #lock (spid int, dbid int, objId int, indId int, Type char(3), Resource varchar(100), Mode varchar(7), Status varchar(10))

insert #lock
exec sp_lock

select spid, db_name(dbid) dbname, objId, indId, Type, Resource, Mode, Status 
from #lock
where mode = 'X'
