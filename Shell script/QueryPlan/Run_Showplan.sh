...
$SYB/isql -S$SRV -U$UserName -P$PW1 <<! > $ProcDir/$procnm
use a2query
go
set nocount on
...
select convert(varchar(40) NULL, c.name) Parm,case  
  when t.name like '%char' then '''A'''when t.name like '%int' then '7' when t.name like '%datetime' then '''02/01/2006''' end ParmValue
into #tmp
...
$SYB/bcp tempdb..execproc out $ProcDir/$procnm -S$SRV -U$UserName -P$PW1 -c -t'|' -r\\n >/dev/null
