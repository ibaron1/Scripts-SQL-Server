 ...
 SELECT TOP (@COUNT) 
    ...
    WHERE attribute = 'dbid' AND  
     CASE when @DBNAME = '<not supplied>' THEN '<not supplied>'
      ...
      AND cp.objtype in ('proc') 
      and DB_NAME(st.dbid)  = DB_NAME()
      and OBJECT_NAME(st.objectid,st.dbid) = 'RefreshMasterAgreementDetails'
    ORDER BY CASE 
         ...
         select [Database Name],[Schema Name],[Object Name], SUM([Total Elasped Time]) as [Elapsed time]
         from #tmp
         group by [Database Name],[Schema Name],[Object Name] 
